...
function interceptorError(error) {
  ...
  if (response.data) {
    ...
    res.message = formattedMessage;
  }
  return res;
}

async function getToken(url, token, idAgencia, idInstitucion) {
  const payload = { token, idAgencia, idInstitucion };
  try {
    const response = await axios.post(url + "/auth/token", payload);
    const { accessToken } = response.data;
    return accessToken;
  } catch (error) {
    throw interceptorError(error);
  }
}
...
async function Consulta(req) {
  ...
  try {
    ...
    let token = await getToken(url, configuracion.getTokenData(), configuracion.getIdAgencia(), configuracion.getIdEntidad());
    let objRequest = new ConsultaRequestModel(req.body);
    objRequest.setUUID(uuid);
    objRequest.setIdInstitucion(configuracion.getIdEntidad());
    objRequest.setIdAgencia(configuracion.getIdAgencia());
    objRequest.setDatosAdicionales({});

    const payload = objRequest.build();
    const headers = { Authorization: `Bearer ${token}` };
    const response = await axios.post(url + "/consulta", payload, { headers });
    const { status, statusText, data } = response;
    if (status == 200) {
      const { message, result, success } = data;
      if (success) {
        let objConsulta = new ConsultaResponseModel(result);
    ...
    objResponse = interceptorError(error);
  }
